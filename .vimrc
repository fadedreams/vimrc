...
" Search for last yanked text (<leader>fy)
" nnoremap <silent> <leader>ff /\V<C-r>=escape(@0, '/\')<CR><CR>
" nnoremap <silent> <leader>gy :call search(getreg('0'))<CR>
" nnoremap <leader>rr :call search(getreg('0'))<CR>
...
" Note: terminals vary — <C-A-j/k> may need remapping if it doesn't fire
...
" Note: this intentionally overwrites the plain :bd mapping above
nnoremap <silent> <leader>bd :call <SID>DeleteBufferAndFile()<CR>

" ── Delete other buffers (spare pinned ones) ──────────────────
" Vimscript has no per-buffer pin table like Lua; we use a global dict instead.
" Toggle pin:  :let g:pinned_buffers[bufnr('%')] = 1  /  :unlet g:pinned_buffers[…]
...
" ── Buffer info float ─────────────────────────────────────────
" Requires Vim 9.0+ (popup_create) — falls back gracefully on older builds.
...
" Listchars — full set (tab, trail, nbsp, eol)
...
" ── Snapshot ──────────────────────────────────────────────────
" Returns dict: { global: {A: {file,lnum,col}, ...}, local: {fname: {a: {lnum,col},...},...} }
function! s:MarksSnapshot()
  ...
  for bnr in range(1, bufnr('$'))
    ...
    for c in range(char2nr('a'), char2nr('z'))
      let mark = nr2char(c)
      let pos  = getpos("'" . mark)
      " pos[1] != 0 means the mark exists; check it belongs to this buffer
      " by comparing the resolved filename
      if pos[1] != 0
...
function! s:MarksJump()
  ...
  " Try global mark via saved JSON first
  if !filereadable(s:marks_save_path) | return | endif
  let raw = join(readfile(s:marks_save_path), '')
  if raw ==# '' | return | endif
  try
...
function! s:MarksShow()
  ...
  " Save marks to disk first so jump reads fresh data
  call s:MarksSave()
